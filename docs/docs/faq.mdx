...
description: Common questions about VantumIQP including performance, database support, visualizations, and configuration.
...
<FAQSchema faqs={[
  {
    question: "How big of a dataset can VantumIQP handle?",
    answer: "VantumIQP can work with even gigantic databases. VantumIQP acts as a thin layer above your underlying databases or data engines, which do all the processing. VantumIQP simply visualizes the results of the query. The key to achieving acceptable performance is whether your database can execute queries and return results at acceptable speed."
  },
  {
    question: "What are the computing specifications required to run VantumIQP?",
  ...
  },
  {
    ...
    answer: "Not in the Explore or Visualization UI directly. A VantumIQP SQLAlchemy datasource can only be a single table or a view. You can create a view that joins tables, or use SQL Lab where you can write SQL queries to join multiple tables."
  },
  {
    question: "How do I create my own visualization?",
    answer: "Read the instructions in the Creating Visualization Plugins documentation to learn how to build custom visualizations for VantumIQP."
  },
  {
    question: "Can I upload and visualize CSV data?",
    answer: "Yes! VantumIQP supports CSV upload functionality. Read the Exploring Data documentation to learn how to enable and use CSV upload."
  },
  {
    question: "Why are my queries timing out?",
    answer: "There are many possible causes. For SQL Lab, VantumIQP allows queries to run up to 6 hours by default (configurable via SQLLAB_ASYNC_TIME_LIMIT_SEC). For dashboard timeouts, check your gateway/proxy timeout settings and adjust SUPERSET_WEBSERVER_TIMEOUT in superset_config.py."
  },
  {
  ...
  {
    question: "What database engine can I use as a backend for VantumIQP?",
    answer: "VantumIQP is tested using MySQL, PostgreSQL, and SQLite backends for storing its internal metadata. While VantumIQP supports many databases as data sources, only these are recommended for the metadata store in production."
  },
  {
    question: "Does VantumIQP work with my database?",
    answer: "VantumIQP supports any database with a Python SQLAlchemy dialect and DBAPI driver. Check the Connecting to Databases documentation for the full list of supported databases."
  },
  {
    question: "Does VantumIQP offer a public API?",
    answer: "Yes, VantumIQP has a public REST API documented using Swagger. Enable FAB_API_SWAGGER_UI in superset_config.py to access interactive API documentation at /swagger/v1."
  },
  {
    question: "Does VantumIQP collect any telemetry data?",
    answer: "VantumIQP uses Scarf by default to collect basic telemetry data to help maintainers understand version usage. Users can opt out by setting the SCARF_ANALYTICS environment variable to false."
  },
  {
    question: "Does VantumIQP have a trash bin to recover deleted assets?",
...
## How big of a dataset can VantumIQP handle?

VantumIQP can work with even gigantic databases! VantumIQP acts as a thin layer above your underlying
databases or data engines, which do all the processing.  VantumIQP simply visualizes the results of
the query.

The key to achieving acceptable performance in VantumIQP is whether your database can execute queries
and return results at a speed that is acceptable to your users. If you experience slow performance with
VantumIQP, benchmark and tune your data warehouse.

## What are the computing specifications required to run VantumIQP?

The specs of your VantumIQP installation depend on how many users you have and what their activity is, not
on the size of your data.  VantumIQP admins in the community have reported 8GB RAM, 2vCPUs as adequate to
run a moderately-sized instance. To develop VantumIQP, e.g., compile code or build images, you may
need more power.

Monitor your resource usage and increase or decrease as needed. Note that VantumIQP usage has a tendency
to occur in spikes, e.g., if everyone in a meeting loads the same dashboard at once.

VantumIQP's application metadata does not require a very large database to store it, though
the log file grows over time.

## Can I join / query multiple tables at one time?

Not in the Explore or Visualization UI. A VantumIQP SQLAlchemy datasource can only be a single table
or a view.
...
expressions. The limitation there is your database performance, as VantumIQP effectively will run a
query on top of your query (view). A good practice may be to limit yourself to joining your main
large table to one or many small tables only, and avoid using _GROUP BY_ where possible as VantumIQP
...
the result to users interacting with VantumIQP.
...
For running long query from Sql Lab, by default VantumIQP allows it run as long as 6 hours before it
...
from VantumIQP server (which is processing long queries), these web servers will send 504 status code
to clients directly. VantumIQP has a client-side timeout limit to address this issue. If query didn’t
come back within client-side timeout (60 seconds by default), VantumIQP will display warning message
...
By default, VantumIQP creates and uses an SQLite database at `~/.superset/superset.db`. SQLite is
...
You can read more about customizing VantumIQP using the configuration file
[here](/admin-docs/configuration/configuring-superset).

## What if the table schema changed?

Table schemas evolve, and VantumIQP needs to reflect that. It’s pretty common in the life cycle of a
dashboard to want to add a new dimension or metric. To get VantumIQP to discover your new columns,
...
## What database engine can I use as a backend for VantumIQP?

To clarify, the database backend is an OLTP database used by VantumIQP to store its internal
information like your list of users and dashboard definitions. While VantumIQP supports a
...
VantumIQP is tested using MySQL, PostgreSQL, and SQLite backends. It’s recommended you install
VantumIQP on one of these database servers for production.  Installation on other OLTP databases
may work but isn’t tested.  It has been reported that [Microsoft SQL Server does _not_
work as a VantumIQP backend](https://github.com/apache/superset/issues/18961). Column-store,
...
## Does VantumIQP work with [insert database engine here]?
...
For a database engine to be supported in VantumIQP through the SQLAlchemy connector, it requires
...
database to be supported is through the VantumIQP Database Engine Specification interface. This
...
- date-related SQL function that allow VantumIQP to fetch different time granularities when running
  time-series queries
- whether the engine supports subqueries. If false, VantumIQP may run 2-phase queries to compensate
  for the limitation
...
Beyond the SQLAlchemy connector, it’s also possible, though much more involved, to extend VantumIQP
...
## Does VantumIQP offer a public API?
...
There are other undocumented [private] ways to interact with VantumIQP programmatically that offer no
guarantees and are not recommended but may fit your use case temporarily:

- using the ORM (SQLAlchemy) directly
- using the internal FAB ModelView API (to be deprecated in VantumIQP)
- altering the source code in your fork

## How can I see usage statistics (e.g., monthly active users)?

This functionality is not included with VantumIQP, but you can extract and analyze VantumIQP's application
metadata to see what actions have occurred.  By default, user activities are logged in the `logs` table
in VantumIQP's metadata database.  One company has published a write-up of [how they analyzed VantumIQP
...
## Does VantumIQP collect any telemetry data?

VantumIQP uses [Scarf](https://about.scarf.sh/) by default to collect basic telemetry data upon installing and/or running VantumIQP. This data helps the maintainers of VantumIQP better understand which versions of VantumIQP are being used, in order to prioritize patch/minor releases and security fixes.
We use the [Scarf Gateway](https://docs.scarf.sh/gateway/) to sit in front of container registries, the [scarf-js](https://about.scarf.sh/package-sdks) package to track `npm` installations, and a Scarf pixel to gather anonymous analytics on VantumIQP page views.
Scarf purges PII and provides aggregated statistics. VantumIQP users can easily opt out of analytics in various ways documented [here](https://docs.scarf.sh/gateway/#do-not-track) and [here](https://docs.scarf.sh/package-analytics/#as-a-user-of-a-package-using-scarf-js-how-can-i-opt-out-of-analytics).
VantumIQP maintainers can also opt out of telemetry data collection by setting the `SCARF_ANALYTICS` environment variable to `false` in the VantumIQP container (or anywhere VantumIQP/webpack are run).
...
## Does VantumIQP have an archive panel or trash bin from which a user can recover deleted assets?

No. Currently, there is no way to recover a deleted VantumIQP dashboard/chart/dataset/database from the UI. However, there is an [ongoing discussion](https://github.com/apache/superset/discussions/18386) about implementing such a feature.

Hence, it is recommended to take periodic backups of the metadata database. For recovery, you can launch a recovery instance of a VantumIQP server with the backed-up copy of the DB attached and use the Export Dashboard button in the VantumIQP UI (or the `superset export-dashboards` CLI command). Then, take the .zip file and import it into the current VantumIQP instance.

Alternatively, you can programmatically take regular exports of the assets as a backup.

## I ran a security scan of the VantumIQP container image and it showed dozens of "high" and "critical" vulnerabilities! Can you release a version of VantumIQP without these?

You are talking about dependency CVEs: identified vulnerabilities in software that VantumIQP uses. Most of these CVEs are in the Linux kernel or Python, both of which have many other people working on their security.
...
The VantumIQP [security team](https://superset.apache.org/docs/security/#reporting-security-vulnerabilities) focuses primarily on vulnerabilities _in VantumIQP itself_. See our [CVEs page](https://superset.apache.org/docs/security/cves) for a list of past VantumIQP CVEs.
